...
Contents: the envelope, errors, exit codes, command metadata, introspection, examples, the audit line, the tenant catalogue, manifests, and alias sets. `$BIN` stands for the binary name of the CLI you are using.
...
| 3 | authentication or authorisation | `NO_CREDENTIALS`, `AUTHENTICATION_FAILED`, a policy that requires identity |
...
Governance reads these labels. A command labelled `read` is never asked to confirm; a command labelled `code_exec` always is. Providers are held to the labels by the conformance suite.
...
| Flag | Returns |
|---|---|
| `--schema` | flags (type, description, required, default, options, char), args, `aciMetadata`, and `availableEntities` from the tenant catalogue when one is cached |
...
Three commands describe the whole CLI: `discover` (every provider with tier, counts, and credential status), `learn <provider>` (a short briefing: overview, topics, key fields, query syntax, patterns, auth paths, the instance's custom entities when a catalogue exists), and `version`. All three are JSON.
...
| `--pretty` | indented and coloured for a person |
...
| `--bootstrap`, `--refresh`, `--all` | on `introspect`: capture, recapture, or widen the tenant catalogue |
