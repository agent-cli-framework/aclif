...
import {CORE_COMMANDS} from '../../cli/define-cli.js'
...
export function catalogSuite(opts: ConformanceOptions): void {
  ...
  describe('C-META-1 and K-3 aciMetadata', () => {
    eachRow(commands, '%s declares complete, valid aciMetadata', (c) => {
      assertAciMetadata(c.cls.aciMetadata, c.id)
    })
  ...
  describe('C-DOC-1, C-DOC-2, C-DOC-3 documentation', () => {
    ...
    it('every provider has a SETUP.md that names the binary as $BIN', () => {
      for (const {plugin, tier} of entries) {
        const file = setupDocPath(opts, plugin.name, tier)
        expect(existsSync(file), rel(file)).toBe(true)
        const text = readFileSync(file, 'utf8')
        expect(text, `${rel(file)} starts with a heading`).toMatch(/^# /)
        expect(text, `${rel(file)} shows how to verify`).toContain('$BIN')
      }
    })

    if (opts.codeowners) {
      const codeowners = opts.codeowners
      it('contributed providers declare maintainers matching their CODEOWNERS line', () => {
        const owners = readFileSync(codeowners, 'utf8').split('\n').filter((l) => l.trim() && !l.startsWith('#'))
        for (const {plugin, tier} of entries) {
  ...
  describe('C-TIER-2 layout', () => {
    it('each source directory holds only provider directories, plus a README and allowlist', () => {
      const byTier = new Map<ProviderTier, string[]>()
      for (const {plugin, tier} of entries) byTier.set(tier, [...(byTier.get(tier) ?? []), plugin.name])
      const seen = new Map<ProviderTier, string[]>()
      for (const {dir, tier} of opts.sourceDirs) {
        const abs = join(opts.cliRoot, dir)
        if (!existsSync(abs)) continue
        for (const name of readdirSync(abs)) {
          if (['README.md', 'dependency-allowlist.json'].includes(name)) continue
          expect(statSync(join(abs, name)).isDirectory(), `${dir}/${name} is not a provider directory`).toBe(true)
          expect(existsSync(join(abs, name, 'plugin.ts')), `${dir}/${name}/plugin.ts`).toBe(true)
          seen.set(tier, [...(seen.get(tier) ?? []), name])
        }
      }
      for (const tier of new Set([...byTier.keys(), ...seen.keys()])) {
        expect((seen.get(tier) ?? []).sort(), `${tier} directories vs registered providers`).toEqual((byTier.get(tier) ?? []).sort())
      }
      if (opts.frameworkSrc) {
        const top = readdirSync(join(opts.frameworkSrc, 'providers')).filter((n) => !['native', 'contributed', 'private'].includes(n)).sort()
